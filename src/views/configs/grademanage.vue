  ...
  <div class="grademanage">
    <el-col class="el-col"> <!-- 头部区域 -->
      ...
      <el-table :data="tableData" width="100%">
        ...
        <el-table-column prop="gNumber" label="年级数量" align="center">
        </el-table-column>
        <el-table-column prop="gradeTotal" label="年级总人数" align="center"></el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="dialogFormVisible" @close="closeDialogAddgsVisible">
      <el-form :model="AddInfo" ref="AddInfo">
        <el-form-item label="年级id" prop="gId" hidden>
          <el-input style="display:inline-block;width:33%;" v-model="AddInfo.gId"></el-input>
        </el-form-item>
        <el-form-item label="年级名称" prop="gradeName" required>
          <el-input style="display:inline-block;width:33%;" v-model="AddInfo.gradeName"></el-input>
        </el-form-item>
        <el-form-item label="年级数量" prop="gNumber" required>
          <el-input style="display:inline-block;width:33%;" v-model="AddInfo.gNumber"></el-input>
        </el-form-item>
        <el-form-item label="年级人数" prop="gradeTotal" required>
          <el-input style="display:inline-block;width:33%;" v-model="AddInfo.gradeTotal"></el-input>
        </el-form-item>
        <el-form-item label="年级等级" prop="gLevel" required>
          <el-input style="display:inline-block;width:33%;" v-model="AddInfo.gLevel"></el-input>
        </el-form-item>
    ...
    methods: {
      ...
      submitForm (AddInfo) {
        ...
        that.$refs[AddInfo].validate(valid => {
          if (valid) {
            Vue.axios.post('/bSysGrade/save', this.AddInfo).then(res => {
      ...
      handleEdit (row, state) {
        if (state == 1) {
          ...
          this.AddInfo.gradeName = row.gradeName
          this.AddInfo.gNumber = row.gNumber
          this.AddInfo.gradeTotal = row.gradeTotal
          this.AddInfo.gLevel = row.gLevel
        } else if (state == 2) {
          this.$confirm('此操作将永久删除该用户, 是否继续?', '提示', {
            ...
            this.tableItem = row
            Vue.axios.post('/bSysGrade/delete', this.tableItem).then(res => {
              let {code, msg} = res.data
              if (!code == 0) {
                this.$set(this.AddInfo, {})
                this.init()
